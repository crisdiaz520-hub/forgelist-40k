...
async function assertRequiredInputs() {
  ...
  for (const directory of directories) {
    if (!(await exists(path.join(root, directory)))) missing.push(`${directory}/`);
  }
  if (missing.length) {
    throw new Error(`Faltan archivos obligatorios en la raiz del repo: ${missing.join(", ")}. Sube esos archivos a GitHub antes de correr el workflow.`);
  }
}
